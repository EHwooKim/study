...
> iframe 태그를 자바스크립트로 어떻게 조작하는지 배웁니다. [코드](./part4/157.html)

* 각 iframe에서 읽는 문서는 독립된 window 객체와 document를 가집니다.
* iframe 태그는 DOM으로 표현하면 HTMLIFrameElement 타입으로 `contentWindow`와 `contentDocument`속성을 가집니다. 두 속성을 통해서 독립된 window 객체와 document 객체에 접근할 수 있습니다.

## iframe과 메세지 교환하기

> iframe 태그에서 불려진 문서에서 현재 문서로 메세지를 전달하는 방법 [코드](./part4/158.html)

* `message 이벤트` : 원본 문서의 window 객체에 message 이벤트에 대해 리스너 함수를 등록하면 iframe 내의 문서에서 `postMessage`로 메세지를 전달할 때 message 이벤트가 발생합니다. 전달한 메시지는 event 파라미터의 data 속성으로 접근이 가능합니다.
* `postMessage`
  * FormData 형식의 객체가 전달되지 않기 때문에 별도의 객체를 정의하여 전달해야 합니다.
  * 메시지를 보낼 대상 window 객체를 통해 호출해야합니다.
    * `window.parent.postMessage` : iframe 태그가 작성되어 있는 부모 window 객체를 가리킵니다.
  * 첫 번째 인자는 전달한 메시지이고 두 번째 인자는 대상 window 객체의 출처를 작성합니다.
    * 출처에 *을 적으면 어떠한 출처도 가리지 않는다는 의미이고 특정  URI를 작성하면 해당 주소에서 호스팅되는 문서의 window 객체로만 메세지를 전달할 수 있습니다.
* `FormData` : XMLHttpRequest를 통해 서버에 데이터를 전달할 떄 사용할 수 있습니다. FormData를 폼 요소 로부터 생성하면 input요소의 name 어트리뷰트를 통해 value 값을 가져올 수 있습니다.

## XMLHttpRequest

> XMLHttpRequest를 사용하여 서버와의 비동기 통신을 처리하는 방법 [코드](./part4/159.html)

* `XMLHttpRequest 객체` 

  * 백그라운드로 서버와의 통신을 할 수 있습니다.
  * XML뿐만 아니라 어떠한 형태의 데이터도 서버로부터 받거나 보낼 수 있습니다. (JSON 메시지 포맷 등)
  * 비동기 처리
  * `XMLHttpRequest` 는 생성자 함수로 new 키워드를 통해 인스턴스를 생성하여 사용합니다.
  * `req.onload` : HTTP요청이 완료되면 호출될 콜백 함수를 정의합니다.요청이 완려되면 onload에 연결된 함수를 호출하는데 이떄 req 객체의 status는 HTTP 상태 코드가 됩니다.
  * `req.onerroe` : 요청을 시도하다 에러 발생 시 호출될 콜백 함수를 정의합니다.
  * `req.open` : HTTP 요청을 초기화합니다. `첫 번쨰 인자로 HTTP 메소드(GET, POST...)`를 `두 번쨰 인자로 URL`을 전달합니다. 세 번쨰 인자로 비동기 여부를 불린 값으로 전달할 수도 있습니다.
  * `req.setRequestHeader` : 요청 헤더를 정의합니다. 반드시 `open` 이후, `send` 이전에 실행시켜야합니다.
  * `req.send` : 실제 요청을 보냅니다.
  * 전달받은 data를 JSON.parse로 객체화하여 활용합니다.

## Fetch API를 활용한 비동기 통신 처리하기

> Promise 기반으로 서버와 HTTP 통신을 할 수 있는 Fetch API에 대해 [코드](./part4/160.html)

* `fetch` : 첫 번쨰 인자로는 `요청할 URL`, 두 번재 인자로는 옵션객체로 헤더와 HTTP 메소드 등을 정의할 수 있습니다.
* 요청이 성공하였을 때 `res.json()` 메소드로 응답 본문을 JSON으로 파싱하여 처리된 결과를 Promise로 반환합니다. Promise로 반환한 응답 분문이 객체로 파싱되어 콜백함수에 전달됩니다. 
* 요청과정에서 에러가 발생할 경우 `catch`로 처리합니다.

## 웹 워커로 백그라운드 코드 실행하기

> 무거운 작업이 스크립트를 메인 동작을 방해하지 않고 백그라운드에서 동작하게 하는 방법 [코드](./part4/161.html)

* `웹 워커`는 무거운 작업의 스크립트를 백그라운드에서 동작할수 있게 합니다. 작업을 수행하는 최소한의 단위를 **스레드**라고 하는데 사용자의 입력이나 화면의 렌더링 등을 다루는 메인 스레드를 방해하지 않고 **별도의 스레드에서 스크립트를 실행**하게 하는 것이 웹 워커입니다.

* `Worker` 
  * new 키워드로 생성하여 사용합니다.
  * 메인 스크립트와 워커 스크립트 간의 메시지 전달은 이벤트 방식으로 동작해서 한쪽에서 `postMessage` 메소드로 메시지를 전달하면 상대편의 `onmessage`에 등록된 콜백 함수를 통해 전달된 메시지를 받을 수 있습니다.
  * 워커 스크립트에서 에러가 발생하면 `onerror` 콜백을 통해 에러를 잡을 수 있습니다.
