...
*/
const go = (...args) => fx.reduce((a, f) => f(a), args) // 하나의 값으로 축약해가는 함수이니 reduce이고, 값이 여러개 들어오니 ...args
                                                        // 초기값을 안주면 알아서 첫번째값이 초기값이 될테고, 보조함수에는 a와 함수 f를 받아 f(a)가 되는 형태겠지
...
*/
const pipe = (f, ...fs) => (...as) => go(f(...as), ...fs) // 일단 함수를 리턴하는 함수, 일단 함수를 받고, 그 이후에 인자 a를 받아 go를 실행시키는 모양
