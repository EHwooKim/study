// replace - 특정 문자열읓 니환하는 방법
console.log('2018-08-03 07-23-14'.replace('-', ':'))   // 일치하는 첫 번째 문자만 바꿉니다.
console.log('2018-08-03 07-23-14'.replace(/-/g, ':'))  // 일치하는 모든 문자를 바꿉니다.
...
console.log(name.replace(/(March) (Amy)/, '$2 $`'))
console.log(name.replace(/(March) (Amy)/, (str, first, second, offset, s) => {
    console.log(`${second} is name, ${first} is frist name.`)
    return `${second} ${first}`
}))