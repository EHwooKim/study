...
app.use(express.json());  // app.use 메서드의 인자로 들어가있는 함수가 미들웨어입니다.  // express 4.16.0 부터 body-parser의 일부기능이 내장되어 이처럼 사용했다.
app.use(express.urlencoded({ extended: false })); // 내부적으로 next()를 호출하여 다음 미들웨어로 넘어가게 됩니다. // 버퍼 데이터, 텍스트 데이터의 경우 내장기능으로 해석이 안되니 body-parser를 설치하여 사용하자.
